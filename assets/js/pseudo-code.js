...
function fetchByCityName(city) {
    ...
    fetch(apiURL)
        ...
        .then(function (data) {
            ...
            fetchByLatLon(lat, lon);
        });
}
function fetchByLatLon(lat, lon) {
    ...
    for (var i = 1; i <= 5; i++) {
        ...
        <div class="card day-0 m-0 text-center">
            <div class="card-body p-1">
                ...
                <p class="card-subtitle mb-2 text-muted">Temp: ${data.daily[i].temp.max
            }<span
                class="fiveDay-temp"></span>
                &deg;F</p>
                <p class="card-subtitle mb-2 text-muted">Humidity: ${data.daily[i].humidity
            } <span
