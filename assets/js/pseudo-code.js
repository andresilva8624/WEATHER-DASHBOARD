...
//     cityListEl.splice.append(cityEl);
//   });
...
$(document).ready(function () {
  ...
  var cityInput = $("#city-input");
  var searchBtn = $("#search-btn");
  var cityListEl = $(".cityList"); // Define cityListEl
  var previousCities = JSON.parse(localStorage.getItem("previousCities")) || [];
  ...
  function renderTodayCard(data, city) {
    ...
    for (var i = 1; i <= 5; i++) {
      var dailyImageURL = `http://openweathermap.org/img/wn/${data.daily[i].weather[0].icon}@2x.png`;
      var cardTemplate = `
        <div class="col-sm-10 col-md-2 offset-lg-1 col-lg-2 p-1">
        <div class="card day-0 m-0 text-center">
            <div class="card-body p-1">
                <p class="card-title">${moment
                  .unix(data.daily[i].dt)
                  .format("MM/DD/YYYY")}</p>
                <img class="fiveDay-img mb-2" src="${dailyImageURL}" />
                <p class="card-subtitle mb-2 text-muted">Temp: ${
                  data.daily[i].temp.max
                }&deg;F</p>
                <p class="card-subtitle mb-2 text-muted">Humidity: ${
  ...
  function displayCities() {
    cityListEl.empty();
    previousCities.slice(-5).forEach(function (city) {
      var cityEl = $(`<div class="city">${city}</div>`);
